...
def anova_tukeyhsd_traffic_injuries(file_path: str):
    ...
    try:
        ...
        return {"result": {}, "metadata": {"error": f"Error reading file: {str(e)}"}}
    ...
    if len(valid_groups) < 2:
        return {
            "result": {},
            "metadata": {"error": "Less than 2 valid groups after filtering."},
        }

    # Extract lists of injuries_total for each traffic_control_device
    data_lists = list(valid_groups.values())
    ...
    ss_within = sum((len(group) - 1) * np.var(group, ddof=1) for group in data_lists)
