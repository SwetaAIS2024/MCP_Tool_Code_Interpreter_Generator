...
def custom_transform_anova_traffic_control_injuries(file_path: str):
    try:
        ...
        if (
            ...
            raise ValueError(
                "CSV file must contain 'traffic_control_device' and 'injuries_total' columns."
            )
        ...
        if len(groups) < 2:
            raise ValueError(
                "At least two different traffic control devices are required for ANOVA."
            )
        ...
        metadata = {"file_path": file_path, "num_devices": len(groups)}
